...
module.exports.initAsync = function (bs, opts, obs) {
    ...
    const watchers = bs.options
        ...
        .map(x => {
            const item = x.toJS();
            const watch = chokidar.watch(item.match, item.options);
    ...
    bs.watchers = watchers.toJS();

    const watchers$ = Rx.Observable.merge(
        bs.watchers.map(item => {
            return watcherAsObservable(item.watcher, item.item);
        })
    ...
    //console.log(bs.watchers.map(item => {
    //    return watcherAsObservable(item.watcher, item.item);
    //}));
    ...
    //const _watchers = allWatchers
    
    //
    //_watchers.subscribe(x => console.log(x.event));

    //// simple function calls. If any files option registered a
    //// callback function, just call it.
    //const watchers$ = _watchers
