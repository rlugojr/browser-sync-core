...
function track(connections$, options$) {
    ...
    connections$
        ...
        .map(x => {
            let current = x.connection.client;

            if (x.clients.hasIn([current.id])) {
                return x.clients.updateIn([current.id], x => updateHeartbeat(x));
            }
            return x.clients.set(current.id, createClient(current));
        })
        .subscribe(clients$);
