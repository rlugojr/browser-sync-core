    ...
    function go (options) {
        ...
        var bs = {
            ...
            setClientOption: function (id, selector, fn) {
                var opts = clientsStreams.clients$.getValue();
                var curr = opts.get(id);
                if (curr) {
                    clientsStreams.clients$.onNext(opts.updateIn([id, 'options'].concat(selector), fn));
                } else {
                    console.log('Client not found'); // todo: decide what to do with missing clients?
                }
            },
            setDefaultClientOption: function (selector, opts, fn) {
