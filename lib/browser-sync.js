...
bs.create = function (userOptions, cb) {
    ...
    function go (options) {
        ...
        /**
         * Create a Subject from the options
         * @type {Rx.SubjectStatic}
         */
        var optSub       = new Rx.Subject(options);
        ...
        bsSocket
            .connections
            .do(client => {
                client.on('Client.register', function (clientData) {

                    var OPT_NAME = ['clientOptions'];
                    var clientPath = OPT_NAME.concat(clientData.id);
                    var toValidate = options.getIn(OPT_NAME.concat(['default']));


                    if (options.hasIn(clientPath)) {
                        client.emit('Options.set', {id: clientData.id, options: options.getIn(clientPath)});
                    } else {
                        client.emit('Options.set', {id: clientData.id, options: toValidate});
            ...
            setClientOption: function (id, selector, fn) {
                return Rx.Observable.just({
                    ...
                    .map(x => {
                        ...
                        if (x.id === 'default') {
                            options = x.options.updateIn(['clientOptions'], x => {
                                return x.map(function (client, key) {
                            ...
                            options
                                ...
                                .forEach(function (x, key) {
                                    if (key !== 'default') {
                                        console.log('sending', key, x);
                                        bsSocket.clients.emit('Options.set', {id: key, options: x});
                                    }
                                });

                        } else {

                            options = x.options.updateIn(['clientOptions', x.id].concat(selector), opt => {
                                return Immutable.fromJS(fn.call(null, opt));
                            });

                            bsSocket.clients.emit('Options.set', {id: x.id, options: options.getIn(['clientOptions', x.id])});
                        }
