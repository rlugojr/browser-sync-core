...
bs.create = function (userOptions, cb) {
    ...
    function go (options) {
        ...
        const options$     = optSub.share();
        const clients      = new Rx.BehaviorSubject(options.get('clientOptions'));
        const clients$     = clients.share().distinctUntilChanged();
        ...
        const clientEvents   = options.get('clientEvents');
        const clientOptions  = transform.getClientOptions(options);
        const clientsStreams = require('./clients').track(bsSocket, optSub);
