...
bs.create = function (userOptions, cb) {
    ...
    function go(options) {
        ...
        //opts$
        //    .map(x => x.get('clientJs'))
        //    .distinctUntilChanged()
        //    .subscribe(applyMw)
        //
        //opts$
        //    .map(x => x.get('middleware'))
        //    .distinctUntilChanged()
        //    .subscribe(applyMw)
        //
        //opts$
        //    .map(x => x.get('rewriteRules'))
        //    .distinctUntilChanged()
        //    .subscribe(applyMw)
        ...
        bs.setOption = function (selector, fn) {
            ...
            return Rx.Observable.just({
                ...
                .withLatestFrom(optSub, (x, opts) => {
                    if (updatableOptions.indexOf(x.selector) > -1) {
                        return opts.updateIn(x.selector.split('.'), (prev) => {
                            console.log(prev);
                            return optionUpdaters[x.selector].call(null, x.fn.call(null, prev.toJS()))
                        });
                    }
                    return opts.updateIn(x.selector, (prev) => x.fn.call(null, prev));
                })
                .do(x => optSub.onNext(x))
                .do(applyMw)
                .take(1);

            // todo: re-apply middlewares that the changed option may need

            //const obs = Rx.Observable.just({
            //    selector: selector,
            //    fn: fn,
