        ...
        .subscribe(options => {
            ...
            var bs = {
                ...
                plugin: function (ns, fn) {
                    ...
                    if (ns === 'client:js') {
                        options = options.update('clientJs', x => fn(x));
                        bsServer.app.stack = bsServer.app.stack.map(x => {
                            if (x.id === 'bs-client' || x.id === 'bs-client-versioned') {
                                x.handle = serveJs(options.get('clientJs').map(item => item.content).join(';'));
                            }
                            return x;
                        });
                    }
