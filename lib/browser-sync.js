    ...
    function go(options) {
        ...
        var bs = {
            ...
            plugin: function (ns, fn) {
                ...
                if (split[0] === 'option') {
                    var optName = split[1];
                    if (Object.keys(optionUpdaters).indexOf(optName) > -1) {
                        options = optionUpdaters[optName](fn);
                        applyMw(options);
                    } else {
                        optName = optName.split('.');
                        options = options.updateIn(optName, x => {
                            if (Immutable.Map.isMap(x) || Immutable.List.isList(x)) {
                                x = x.toJS();
                            }
                            return Immutable.fromJS(fn(x));
                        });
                    }
                }
